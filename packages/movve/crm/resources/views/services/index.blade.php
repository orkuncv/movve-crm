    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <h3 class="text-lg font-bold mb-4">{{ __('crm::crm.services_overview') }}</h3>
                <div class="space-y-6">
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg shadow flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                            <div>
                                ...
                                <div class="text-gray-500 text-sm mt-1">
                                    {{ __('crm::crm.category') }} {{ $service->category?->name ?? '-' }}
                                    | {{ __('crm::crm.price') }} {{ $service->price ? ('€ ' . number_format($service->price, 2, ',', '.')) : '-' }}
                                    | {{ __('crm::crm.duration') }} {{ $service->duration ? ($service->duration . ' min') : '-' }}
                                </div>
                                <div class="text-gray-500 text-xs mt-1">
                                    {{ __('crm::crm.staff_members') }}
                    ...
                    @empty
                        <div class="text-gray-500 italic">{{ __('crm::crm.no_services') }}</div>
                    @endforelse
                </div>
            </div>
        </div>
