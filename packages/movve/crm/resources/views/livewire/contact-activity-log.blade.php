<div>
    <div class="bg-white overflow-hidden sm:rounded-lg p-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('Activiteitenlogboek') }}</h3>
                        ...
                        @if($activity->action === 'updated' && !empty($activity->properties))
                            <div class="mt-2 text-xs">
                                <details class="cursor-pointer">
                                    <summary class="text-blue-600 hover:text-blue-800">{{ __('Toon wijzigingen') }}</summary>
        ...
        @else
            <div class="text-gray-500 text-sm italic">
                {{ __('Geen activiteiten gevonden voor dit contact.') }}
            </div>
        @endif
    </div>
</div>
